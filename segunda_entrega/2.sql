...
DROP FUNCTION IF EXISTS ES_BICIESTO;

DELIMITER $$
CREATE FUNCTION ES_BICIESTO(anio INT)
RETURNS BOOLEAN
DETERMINISTIC
BEGIN
...
BEGIN
    ...
    SET FECHA_INICIO_MES = STR_TO_DATE(CONCAT(P_ANIO, '-', P_MES, '-01'), '%Y-%m-%d'); 
    IF P_MES = 2 THEN
		IF ES_BICIESTO(P_ANIO) THEN 
			SET FECHA_FIN_MES = STR_TO_DATE(CONCAT(P_ANIO, '-', P_MES, '-29'), '%Y-%m-%d'); 
		ELSE
			SET FECHA_FIN_MES = STR_TO_DATE(CONCAT(P_ANIO, '-', P_MES, '-28'), '%Y-%m-%d');
		END IF;
    ELSE
		CASE 
			WHEN P_MES = '04' OR P_MES = '06' OR P_MES = '09' OR P_MES = '11' THEN
				SET FECHA_FIN_MES = STR_TO_DATE(CONCAT(P_ANIO, '-', P_MES, '-30'), '%Y-%m-%d');
			WHEN P_MES = '01' OR P_MES = '03' OR P_MES = '05' OR P_MES = '07' OR P_MES = '08' OR P_MES = '10' THEN
				SET FECHA_FIN_MES = STR_TO_DATE(CONCAT(P_ANIO, '-', P_MES, '-31'), '%Y-%m-%d');
			ELSE
				SET FECHA_FIN_MES = NULL;
		END CASE;
	END IF;
    ...
    INTO TOTAL_VENTAS
    FROM FACTURA
    WHERE FACTURA.FECHA_FACTURA >= FECHA_INICIO_MES AND FACTURA.FECHA_FACTURA <= FECHA_FIN_MES;
...
DELIMITER $$
CREATE PROCEDURE SP_DETALLE_VENTAS(INOUT FECHA_INICIO_PERIODO DATE, INOUT FECHA_FIN_PERIODO DATE, OUT CANTIDAD_VENTAS INT, OUT PROMEDIO_VENTAS DECIMAL(10,2))
BEGIN
	SELECT COUNT(*)
    INTO CANTIDAD_VENTAS
    FROM FACTURA
    WHERE FACTURA.FECHA_FACTURA BETWEEN FECHA_INICIO_PERIODO AND FECHA_FIN_PERIODO;
    
    SELECT AVG(FACTURA.TOTAL)
    INTO PROMEDIO_VENTAS
    FROM FACTURA 
	WHERE FACTURA.FECHA_FACTURA BETWEEN FECHA_INICIO_PERIODO AND FECHA_FIN_PERIODO;
...
DELIMITER $$
CREATE PROCEDURE SP_PROMEDIO_DE_CALIFICACION_DE_PRODUCTO (IN ID_PRODUCTO_IN INT, OUT PROMEDIO DECIMAL(2,1), OUT NOMBRE_PRODUCTO VARCHAR(80))
BEGIN
    ...
    WHERE PRODUCTO.ID_PRODUCTO = ID_PRODUCTO_IN;
...
-- PORQUE DEVUELVE MAS DE UN RESULTADO
-- ------------------------------------------------------
DROP PROCEDURE IF EXISTS SP_OBTENER_VENTAS_POR_MES_DE_ANIO;
DELIMITER $$
CREATE PROCEDURE SP_OBTENER_VENTAS_POR_MES_DE_ANIO(IN P_ANIO INT)
BEGIN 
	DECLARE MES_INICIO INT;
    DECLARE MES_FIN INT;
    DECLARE SUMA_VENTAS DECIMAL(10,2);
    
    SET MES_INICIO = 1;
    SET MES_FIN = 12;
    
    WHILE MES_INICIO <= MES_FIN DO
		SELECT SUM(FACTURA.TOTAL) INTO SUMA_VENTAS
        FROM FACTURA
        WHERE YEAR(FECHA_FACTURA) = P_ANIO
        AND MONTH(FECHA_FACTURA) = MES_INICIO;
        
		SELECT CONCAT('Ventas del mes ', mes_inicio, ' del año ', P_anio, ': ', IFNULL(suma_ventas, 0)) AS resultado;
...
-- CALL SP_OBTENER_VENTAS_POR_MES_DE_ANIO(2024);
-- -----------------------------------------------
-- CON ESTE SELECT TRAIGO TODAS LAS VENTAS DEL MES
-- -----------------------------------------------


-- SELECT * FROM FACTURA;
-- SELECT FN_VENTAS_DEL_MES('05','2024'); -- 8 DEBE TRAER
